...
try:
    import solana_payments
    SOLANA_ENABLED = True
except ImportError as e:
    logger.error(f"Failed to import solana_payments: {e}")
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        elif data == "admin_leaderboard_add_messages":
        ...
        elif data == "admin_leaderboard_reset_confirm":
        ...
        elif data == "admin_leaderboard_reset_execute":
            await admin_panel.leaderboard_reset_execute(query, context)
        elif data == "admin_leaderboard_view":
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type == 'private':
        ...
        # Check if awaiting payment input (deposit amount or withdrawal details)
        if await payments.handle_payment_text(update, context):
            return
...
def create_application():
    ...
    # Exchange rate management command
    @group_whitelist_check
    async def setexchangerate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin command to set exchange rate (points per dollar)"""
        # Check if user is owner
        if update.effective_user.id != OWNER_ID:
            await update.message.reply_text("❌ Tik savininkas gali keisti keitimo kursą!")
            return
        
        # Check if rate is provided
        if not context.args or len(context.args) != 1:
            current_rate = database.get_exchange_rate()
            await update.message.reply_text(
                f"📊 <b>Keitimo Kursas</b>\n\n"
                f"Dabartinis kursas: <b>{current_rate:,} taškų = $1.00</b>\n\n"
                f"Norėdami pakeisti, naudokite:\n"
                f"<code>/setexchangerate [taškai]</code>\n\n"
                f"Pavyzdys: <code>/setexchangerate 2500</code>",
                parse_mode='HTML'
            )
            return
        ...
        try:
            ...
            if new_rate < 100 or new_rate > 100000:
                await update.message.reply_text("❌ Kursas turi būti tarp 100 ir 100,000 taškų/$1")
                return
            
            # Update rate
            success = database.set_exchange_rate(new_rate, update.effective_user.id)
            
            if success:
                await update.message.reply_text(
                    f"✅ <b>Kursas pakeistas!</b>\n\n"
                    f"Naujas kursas: <b>{new_rate:,} taškų = $1.00 USD</b>\n\n"
                    f"💡 Visi mainai nuo šiol naudos naują kursą.",
                    parse_mode='HTML'
                )
                logger.info(f"Admin {update.effective_user.id} changed exchange rate to {new_rate}")
            else:
                await update.message.reply_text("❌ Klaida keičiant kursą. Patikrinkite logs.")
        
        except ValueError:
            await update.message.reply_text("❌ Neteisingas formatas. Naudokite skaičių.\n\nPavyzdys: <code>/setexchangerate 2500</code>", parse_mode='HTML')
    
    application.add_handler(CommandHandler("setexchangerate", setexchangerate_command))
    ...
    application.add_handler(CommandHandler("stats", leaderboard.leaderboard_command))  # Top chatters leaderboard
    ...
    # Payment callbacks (deposit/withdraw) - Updated for Solana
    application.add_handler(CallbackQueryHandler(
        payments.handle_payment_callback,
        pattern="^(deposit|withdraw|sol_deposit|sol_withdraw|cancel_deposit_|confirm_withdraw_|cancel_withdraw)"
    ))
...
async def main():
    ...
    if SOLANA_ENABLED:
        try:
            logger.info("⚡ Initializing Solana payment system...")
            solana_payments.init_solana_config()
            # Pre-fetch SOL price to populate cache
            await solana_payments.prefetch_sol_price()
            logger.info("✅ Solana payment system ready")
        except Exception as e:
            logger.error(f"❌ Failed to initialize Solana: {e}")
    else:
        logger.warning("⚠️ Solana payment system not available")
    ...
    # Start HTTP server for webhooks (keeping for backwards compatibility)
    ...
    try:
        ...
        # Store bot instance for scheduled jobs (MUST be after bot starts, while event loop is running)
        logger.info("💾 Storing bot instance for scheduled jobs...")
        recurring_messages.set_bot_instance(application.bot)
        
        # Start Solana deposit monitoring (background task)
        if SOLANA_ENABLED:
            logger.info("🔍 Starting Solana deposit monitoring...")
            application.create_task(solana_payments.start_deposit_monitoring(application))
