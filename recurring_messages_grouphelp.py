...
_bot_instance = None
_main_event_loop = None  # Store main event loop for thread-safe async calls
...
def set_bot_instance(bot):
    ...
    global _bot_instance, _main_event_loop
    import asyncio
    _bot_instance = bot
    # Store the current event loop (the main bot's event loop)
    try:
        _main_event_loop = asyncio.get_running_loop()
        logger.info("✅ Bot instance and event loop stored for recurring messages")
    except RuntimeError:
        logger.warning("⚠️ No running event loop found during initialization")
...
def send_recurring_message_sync(chat_id: int, message_id: int):
    ...
    Uses run_coroutine_threadsafe to schedule in main event loop (REUSES connection pool)
    """
    import asyncio
    ...
    try:
        if _main_event_loop is None or _main_event_loop.is_closed():
            logger.error("❌ Main event loop not available. Cannot send recurring message.")
            return
        
        # Schedule coroutine in main event loop (REUSES bot's connection pool)
        future = asyncio.run_coroutine_threadsafe(
            _send_recurring_message_async(chat_id, message_id), 
            _main_event_loop
        )
...
def schedule_barygos_auto_job():
    ...
    def sync_wrapper():
        ...
        try:
            if _main_event_loop is None or _main_event_loop.is_closed():
                logger.error("❌ Main event loop not available. Cannot send barygos auto-post.")
                return
            
            # Schedule coroutine in main event loop (REUSES bot's connection pool)
            future = asyncio.run_coroutine_threadsafe(send_barygos_to_groups(), _main_event_loop)
            # Wait for completion (with timeout)
            future.result(timeout=30)
        except Exception as e:
            logger.error(f"Error in barygos auto-post job: {e}", exc_info=True)
